...
if ($myRole === 'svp' || $myRole === 'sysadmin') {
    $dashWhere = "1=1"; // SVPs and sysadmin see the whole company
} elseif ($myRole === 'vp') {
    $dashWhere = "w.vp_id = ?";
    $dashParams = [$myId];
...
// Find personal details — sysadmin has no real employee row so handle gracefully
$myDetails = ['first_name' => 'Sys', 'last_name' => 'Admin', 'role' => 'Sysadmin', 'title' => 'System Administrator', 'tenure' => null, 'anniversary' => null, 'organization_name' => null, 'work_city' => null, 'state' => null, 'manager_first' => null, 'manager_last' => null];
...
if ($myRole !== 'employee') {
    ...
    if (in_array($myRole, ['vp', 'svp', 'sysadmin'])) {
        $stmt = $pdo->prepare("
    ...
    if (in_array($myRole, ['director', 'vp', 'svp', 'sysadmin'])) {
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .leaderboard-list { display: flex; flex-direction: column; gap: 12px; max-height: 280px; overflow-y: auto; padding-right: 8px; }
...
<div class="dashboard-container">
  ...
  <div class="card card-top-accent">
    <div class="snapshot-header">
      <div class="snapshot-avatar"><?php echo substr($myDetails['first_name'],0,1) . substr($myDetails['last_name'],0,1); ?></div>
      <div>
        <div class="snapshot-name"><?php echo htmlspecialchars($myDetails['first_name'] . ' ' . $myDetails['last_name']); ?></div>
  ...
  <div class="kpi-grid">
    <div class="kpi-card">
      <div class="kpi-value"><?php echo number_format($totalEmployees); ?></div>
      <div class="kpi-label">Total Employees</div>
    </div>
    <?php if (!in_array($myRole, ['manager'])): ?>
    <div class="kpi-card">
      <div class="kpi-value"><?php echo number_format($totalManagers); ?></div>
      <div class="kpi-label">Managers & Leaders</div>
    </div>
    <?php endif; ?>
    <div class="kpi-card">
      <div class="kpi-value"><?php echo $avgTenure; ?></div>
      <div class="kpi-label">Avg Tenure (Yrs)</div>
    </div>
    <div class="kpi-card" style="border-bottom-color: var(--orange);">
      <div class="kpi-value"><?php echo number_format($totalLocations); ?></div>
  ...
  <div class="chart-grid">
      ...
      <div class="chart-container chart-standard">
        <div class="leaderboard-list">
          ...
          <div class="leaderboard-item">
            <div class="leaderboard-label" title="<?php echo htmlspecialchars($s['state']); ?>"><?php echo htmlspecialchars($s['state']); ?></div>
            <div class="leaderboard-bar-track"><div class="leaderboard-bar-fill" style="width:<?php echo $pct; ?>%"></div></div>
            <div class="leaderboard-value"><?php echo number_format($s['count']); ?></div>
          </div>
          <?php endforeach; ?>
          <?php if (empty($stateData)): ?><div style="color:var(--muted);font-size:13px;">No data available.</div><?php endif; ?>
        </div>
      </div>
    </div>

    <?php if (in_array($myRole, ['vp', 'svp', 'sysadmin'])): ?>
    <div class="card">
    ...
    <?php if (in_array($myRole, ['director', 'vp', 'svp', 'sysadmin'])): ?>
    <div class="card">
      ...
      <div class="chart-container chart-standard">
        <div class="leaderboard-list">
          ...
          ?>
          <div class="leaderboard-item">
            <div class="leaderboard-label" title="<?php echo htmlspecialchars($d['organization_name']); ?>"><?php echo htmlspecialchars($d['organization_name']); ?></div>
            <div class="leaderboard-bar-track"><div class="leaderboard-bar-fill" style="width:<?php echo $pct; ?>%"></div></div>
            <div class="leaderboard-value"><?php echo number_format($d['count']); ?></div>
          </div>
          <?php endforeach; ?>
          <?php if (empty($deptData)): ?><div style="color:var(--muted);font-size:13px;">No data available.</div><?php endif; ?>
        </div>
      </div>
    </div>
    <?php endif; ?>
...
<?php if (in_array($myRole, ['manager', 'director'])): ?>
const titleData = <?php echo json_encode($titleData); ?>;
new Chart(document.getElementById('titleChart'), {
    type: 'doughnut',
    data: { labels: titleData.map(d => d.title || 'Unknown'), datasets: [{ data: titleData.map(d => d.count), backgroundColor: palette, borderWidth: 0 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right' }, tooltip: percentageTooltipConfig } }
});
<?php endif; ?>

const payBandData = <?php echo json_encode($payBandData); ?>;
new Chart(document.getElementById('payBandChart'), {
    type: 'bar',
    data: { labels: payBandData.map(d => d.pay_band || 'Unknown'), datasets: [{ label: 'Employees', data: payBandData.map(d => d.count), backgroundColor: brandPurple, borderRadius: 4 }] },
...
new Chart(document.getElementById('roleChart'), {
    type: 'doughnut',
    data: { labels: roleData.map(d => d.role || 'Unknown'), datasets: [{ data: roleData.map(d => d.count), backgroundColor: [brandOrange, brandPurple, '#1a56c4', '#009966', '#e834eb'], borderWidth: 0 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right' }, tooltip: percentageTooltipConfig } }
});
<?php endif; ?>

<?php if ($myRole === 'director'): ?>
const empPerMgrData = <?php echo json_encode($empPerMgrData); ?>;
new Chart(document.getElementById('empPerMgrChart'), {
    type: 'bar',
    data: { labels: empPerMgrData.map(d => d.manager_name || 'Unknown'), datasets: [{ label: 'Employees on Team', data: empPerMgrData.map(d => d.count), backgroundColor: '#1a56c4', borderRadius: 4 }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
});
<?php endif; ?>
